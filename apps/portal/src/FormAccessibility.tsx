...
export default function FormAccessibility() {
  ...
  useEffect(() => {
    ...
    return () => { observer.disconnect(); document.removeEventListener("focusout", blur, true); document.removeEventListener("input", input, true); document.removeEventListener("submit", submit, true); window.removeEventListener("fisiofit:api-error", apiError); };
  }, []);
