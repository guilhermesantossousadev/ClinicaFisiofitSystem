...
export default function PrivacyPage() {
  return <Layout><PageHero eyebrow="Privacidade" title="Política de Privacidade" description="Como a Clínica Fisiofit protege e utiliza dados pessoais." /><div className="site-section-compact"><div className="site-reading-container legal-content"><div role="status" className="rounded-2xl border border-amber-300 bg-amber-50 p-5 text-sm leading-relaxed text-amber-950"><strong>Documento em validação:</strong> a razão social, o CNPJ e o e-mail dedicado de privacidade serão inseridos antes da publicação em produção.</div>{sections.map(([title,text])=><section key={title}><h2>{title}</h2><p>{text}</p></section>)}<section><h2>Controladora e contato</h2><p>Controladora: Clínica Fisiofit — razão social e CNPJ pendentes de confirmação. Canal de privacidade: endereço dedicado pendente de confirmação.</p></section><p className="border-t border-line pt-6 text-xs">Versão 1.0 — atualizada em 2 de agosto de 2026.</p></div></div></Layout>;
}
